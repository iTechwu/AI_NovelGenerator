...
Before architecture-affecting work, read the relevant rule file in
`docs/shared-agent-rules/`:

- `core-architecture-rules.md` for DB, client, logging, and scaffold genericity
- `api-patterns.md` for ts-rest, list endpoints, proxy, and product-level actions
- `zod4-guidelines.md` for Zod 4 contract rules
- `module-dependency-rules.md` for API/domain/infra dependency direction
- `cron-queue-rules.md` for Cron and BullMQ queue registration
- `cross-project-boundaries.md` for SSO/infra/models ownership
- `workflow-api-patterns.md` for long-running workflow and state-machine APIs
...
└── packages/                   # Shared packages (frontend + backend)
    ├── ui/                     # @repo/ui - local UI extension layer (shadcn/ui)
...
node_modules/@dofe/infra-*      # Infra packages consumed from npm
```
...
- `@repo/ui` - Local UI extension layer. Do not fork `@dofe/sso-ui` into this package; consume SSO-owned auth/account UI through `@dofe/sso-ui` or a documented adapter when a project needs it.
...
- **infra** (`@dofe/infra-*`): 已从 `libs/infra/` 提取至独立仓库 `infra.dofe.ai`，本脚手架通过 npm 包消费，不再要求同级 workspace。包含 common, clients, prisma, redis, rabbitmq, jwt, utils, i18n, shared-db, shared-services。与产品/领域无关，可复用。**禁止**依赖 `libs/domain/**`。
