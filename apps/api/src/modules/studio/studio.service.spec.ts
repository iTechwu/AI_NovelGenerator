...
describe("StudioService", () => {
  ...
  it("persists a project and queued run before dispatching the same run id to Python", async () => {
    ...
    expect(runtimeClient.createJob.mock.calls[0][2]).toBe(
      runService.create.mock.calls[0][0].id,
    );
  });
  ...
  it("lists only the current author projects with their latest persisted run", async () => {
    ...
    await expect(
      service.listProjects(ownerId, { page: 1, limit: 20 }),
    ).resolves.toEqual({
      list: [
        {
          id: projectId,
  ...
  it("returns the latest versioned blueprint only after verifying project ownership", async () => {
    ...
    await expect(
      service.getBlueprint(ownerId, projectId),
  ...
  it("confirms the latest blueprint and updates the project pointer in one unit of work", async () => {
    ...
    await expect(
      service.confirmBlueprint(ownerId, projectId),
  ...
  it("confirms the latest chapter plan without changing its content", async () => {
    ...
    await expect(
      service.confirmChapterPlan(ownerId, projectId, 1),
    ).resolves.toMatchObject({
      status: "confirmed",
      title: "信件抵达",
    });
  ...
  it("queues a chapter draft only from the confirmed plan bound to the current blueprint", async () => {
    ...
    chapterPlanService.list.mockResolvedValue({
      list: [confirmedPlan], total: 1, page: 1, limit: 1,
    });
    runService.create.mockResolvedValue({ id: runId });
    runService.update.mockResolvedValue({
      id: runId, projectId, chapterPlanId: planId, type: "CHAPTER_DRAFT",
      status: "QUEUED", progress: 0, currentStep: "Queued for chapter draft",
      architecture: null, outline: null, chapterContent: null, error: null,
      createdAt, updatedAt: createdAt,
    });
    runtimeClient.createChapterDraftJob.mockResolvedValue({
      id: runId, project: { id: projectId, ...projectInput }, status: "queued",
      progress: 0, currentStep: "Queued for chapter draft",
    ...
    expect(runService.create).toHaveBeenCalledWith(expect.objectContaining({
      type: "CHAPTER_DRAFT", chapterPlan: { connect: { id: planId } },
      inputSummary: "开场先写雨声。",
    }));
    expect(runtimeClient.createChapterDraftJob).toHaveBeenCalledWith(
      ownerId, projectId, expect.any(String), projectInput,
  ...
  it("restores an immutable revision by moving only the current draft pointer", async () => {
    const revision = {
      id: runId, projectId, chapterPlanId: "227dd8ce-b405-4609-bffc-e88f8842e1ab",
      runId, chapterNumber: 1, version: 1, status: "DRAFT", content: "雨声先于信件抵达。",
      wordCount: 1, promptSummary: "", source: "ai", schemaVersion: 1,
      contentHash: "a".repeat(64), createdAt, updatedAt: createdAt,
    };
    ...
    chapterDraftPointerService.upsert.mockResolvedValue({ id: "98cc0808-e3d3-4313-b276-3c6608168f0a" });
  ...
  it("returns a line-level immutable diff for two revisions of the same chapter", async () => {
    const baseRevision = {
      id: runId, projectId, chapterPlanId: "227dd8ce-b405-4609-bffc-e88f8842e1ab",
      runId, chapterNumber: 1, content: "雨声。\n信件抵达。\n", version: 1,
    };
  ...
  it("creates a proposed fact change without writing to the confirmed fact layer", async () => {
    ...
    const change = {
      id: "ad6cbd8b-a144-41cb-b1d4-256b196f9c9c", projectId, revisionId: runId,
      chapterNumber: 1, factId: null, operation: "ADD", factType: "character",
      subject: "林雾", predicate: "knows", proposedValue: "旧案线索", rationale: "正文提及", evidence: "她握紧信件。",
      source: "author", status: "PROPOSED", resolvedValue: null, resolvedAt: null, createdAt, updatedAt: createdAt,
    };
    projectService.getById.mockResolvedValue({ id: projectId, ownerId, ...projectInput });
    chapterRevisionService.getById.mockResolvedValue(revision);
    factChangeService.create.mockResolvedValue(change);

    await expect(service.createFactChange(ownerId, projectId, 1, runId, {
      operation: "add", factType: "character", subject: "林雾", predicate: "knows",
      proposedValue: "旧案线索", rationale: "正文提及", evidence: "她握紧信件。",
    })).resolves.toMatchObject({ status: "proposed", subject: "林雾" });
    expect(factService.create).not.toHaveBeenCalled();
    expect(factChangeService.create).toHaveBeenCalledWith(expect.objectContaining({
      status: "PROPOSED", source: "author", operation: "ADD",
    }));
  });

  it("lists only confirmed facts as fact-change targets", async () => {
    const fact = {
      id: "ad6cbd8b-a144-41cb-b1d4-256b196f9c9c", projectId, factType: "character",
      subject: "林雾", predicate: "knows", value: "旧案线索", effectiveChapter: 1,
      status: "CONFIRMED", createdAt, updatedAt: createdAt,
    };
    projectService.getById.mockResolvedValue({ id: projectId, ownerId, ...projectInput });
    factService.list.mockResolvedValue({ list: [fact], total: 1, page: 1, limit: 100 });

    await expect(service.listFacts(ownerId, projectId, { page: 1, limit: 100 }))
  ...
  it("accepts an add proposal by creating a confirmed fact and closing the proposal", async () => {
    const change = {
      id: "ad6cbd8b-a144-41cb-b1d4-256b196f9c9c", projectId, revisionId: runId,
      chapterNumber: 1, factId: null, operation: "ADD", factType: "character",
      subject: "林雾", predicate: "knows", proposedValue: "旧案线索", rationale: "", evidence: "",
      source: "ai", status: "PROPOSED", resolvedValue: null, resolvedAt: null, createdAt, updatedAt: createdAt,
    };
    const accepted = { ...change, factId: "98cc0808-e3d3-4313-b276-3c6608168f0a", status: "ACCEPTED", resolvedValue: "旧案线索", resolvedAt: createdAt };
    projectService.getById.mockResolvedValue({ id: projectId, ownerId, ...projectInput });
    ...
    factService.create.mockResolvedValue({ id: accepted.factId });
    factChangeService.update.mockResolvedValue(accepted);

    await expect(service.resolveFactChange(ownerId, projectId, 1, runId, change.id, {
      decision: "accept",
    })).resolves.toMatchObject({ status: "accepted", resolvedValue: "旧案线索" });
    expect(factService.create).toHaveBeenCalledWith(expect.objectContaining({
      sourceChange: { connect: { id: change.id } }, status: "CONFIRMED", value: "旧案线索",
    }));
    expect(factChangeService.update).toHaveBeenCalledWith(
      { id: change.id },
      expect.objectContaining({ status: "ACCEPTED", resolvedValue: "旧案线索" }),
    );
  });
  ...
  it("persists AI fact proposals from a completed chapter draft exactly once", async () => {
    ...
    const aiProposal = {
      operation: "add", factType: "character", subject: "林雾", predicate: "knows",
      proposedValue: "旧案线索", rationale: "正文明确提及", evidence: "林雾握紧信件。", confidence: 0.9,
    };
    projectService.getById.mockResolvedValue({ id: projectId, ownerId, ...projectInput });
    runService.getById.mockResolvedValue({
      id: runId, projectId, chapterPlanId, type: "CHAPTER_DRAFT", status: "RUNNING",
      progress: 70, currentStep: "Generating chapter draft", architecture: null, outline: null,
      chapterContent: null, factChanges: [], error: null, createdAt, updatedAt: createdAt,
    });
    runtimeClient.getJob.mockResolvedValue({
      id: runId, project: { id: projectId, ...projectInput }, status: "succeeded", progress: 100,
      currentStep: "Generation complete", artifact: { chapterDraft: "林雾握紧信件。", factChanges: [aiProposal] },
      createdAt: createdAt.toISOString(), updatedAt: createdAt.toISOString(),
    });
    runService.update.mockResolvedValue({
      id: runId, projectId, chapterPlanId, type: "CHAPTER_DRAFT", status: "SUCCEEDED",
      progress: 100, currentStep: "Generation complete", architecture: null, outline: null,
      chapterContent: "林雾握紧信件。", factChanges: [aiProposal], modelConfig: {}, error: null,
      inputSummary: "", createdAt, updatedAt: createdAt,
    });
    ...
    chapterRevisionService.create.mockResolvedValue({
      id: runId, projectId, chapterNumber: 1, chapterPlanId, runId,
    });
    chapterDraftPointerService.upsert.mockResolvedValue({ id: "98cc0808-e3d3-4313-b276-3c6608168f0a" });
    ...
    expect(factChangeService.create).toHaveBeenCalledWith(expect.objectContaining({
      revision: { connect: { id: runId } }, source: "ai", status: "PROPOSED",
      subject: "林雾", evidence: "林雾握紧信件。", confidence: 0.9,
    }));
  });
});
