 ...
 *
 * 注意：@dofe/infra-* 包通过 npm 依赖消费，不再向模板注入同级
 * infra.dofe.ai workspace 引用。
 *
 * 使用：在仓库根目录执行
...
const REQUIRED_TEMPLATE_ADDITIONS = [
  'docs/0628/',
...
function main() {
  ...
  function collectFiles(rel) {
    ...
    return fs.readdirSync(full, { withFileTypes: true }).flatMap((entry) => {
      const child = path.posix.join(rel.replace(/\/$/, ''), entry.name);
      if (entry.isDirectory()) return collectFiles(`${child}/`);
      return entry.isFile() ? [child] : [];
    });
  }

  for (const rel of REQUIRED_TEMPLATE_ADDITIONS.flatMap(collectFiles)) {
    if (!files.includes(rel) && !rel.startsWith(EXCLUDE_PREFIX) && !EXCLUDE_PATTERNS.some((pat) => rel.startsWith(pat))) {
      files.push(rel);
    }
  }
