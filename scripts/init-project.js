...
function getScriptedAnswers() {
  if (scriptedAnswers !== null) return scriptedAnswers;
...
// Promisify question. When stdin is piped, consume answers line-by-line so
// smoke checks can exercise the same initialization path without a TTY.
const question = (query) => {
  const answers = getScriptedAnswers();
  if (answers.length > 0) {
    process.stdout.write(query);
    const answer = answers[scriptedAnswerIndex] ?? '';
...
const config = {
  ...
  redisUrl: '',
  rabbitmqUrl: '',
  webApiUrl: 'http://127.0.0.1:13100',
...
async function main() {
  ...
  try {
    ...
    config.webApiUrl = await question(
      `${colors.cyan}Web API URL${colors.reset} [http://127.0.0.1:13100]: `,
    );
    config.webApiUrl = config.webApiUrl.trim() || 'http://127.0.0.1:13100';

    config.ssoBaseUrl = await question(
      `${colors.cyan}SSO Base URL${colors.reset} [http://127.0.0.1:3100]: `,
    );
    config.ssoBaseUrl = config.ssoBaseUrl.trim() || 'http://127.0.0.1:3100';
    ...
    console.log(`  Web API URL:     ${config.webApiUrl}`);
    console.log(`  SSO Base URL:    ${config.ssoBaseUrl}`);
...
function createEnvFromExample(rootDir, config) {
  ...
  const apiReplacements = {
    ...
    SSO_API_URL: config.ssoBaseUrl,
    SSO_INTERNAL_API_URL: config.ssoBaseUrl,
    SSO_ISSUER: config.ssoBaseUrl,
  };
  ...
  const webReplacements = {
    NEXT_PUBLIC_SERVER_BASE_URL: config.webApiUrl,
    SCAFFOLD_INTERNAL_API_URL: config.webApiUrl,
    NEXT_PUBLIC_SSO_BASE_URL: config.ssoBaseUrl,
  };
