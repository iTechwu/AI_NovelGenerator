...
3. **Install dependencies and run the internal Runtime**
    - Open a terminal and change to the project directory:
       ```bash
       cd backend
       ```
    - Install the locked Runtime and test dependencies with uv:
       ```bash
       uv sync --group dev
       ```
    - Vector retrieval and the legacy desktop UI remain optional on Python
      3.14. Install them only when required:
       ```bash
       uv sync --extra retrieval
       uv sync --extra desktop
       ```
    - Configure the internal caller secret and start FastAPI:
       ```bash
       export NOVEL_RUNTIME_SHARED_SECRET='replace-with-a-long-random-secret'
       uv run uvicorn runtime.api:app --host 127.0.0.1 --port 18080
       ```

To run the Runtime test suite:
```bash
uv run pytest
```
